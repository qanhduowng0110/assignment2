...
func (etq *EventTypeQuery) QueryEarthquake() *EarthquakeQuery {
	query := (&EarthquakeClient{config: etq.config}).Query()
...
func (etq *EventTypeQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (etq *EventTypeQuery) FirstIDX(ctx context.Context) int32 {
...
func (etq *EventTypeQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (etq *EventTypeQuery) OnlyIDX(ctx context.Context) int32 {
...
func (etq *EventTypeQuery) IDs(ctx context.Context) (ids []int32, err error) {
...
func (etq *EventTypeQuery) IDsX(ctx context.Context) []int32 {
	ids, err := etq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (etq *EventTypeQuery) Clone() *EventTypeQuery {
	...
	return &EventTypeQuery{
		...
		withEarthquake: etq.withEarthquake.Clone(),
...
func (etq *EventTypeQuery) WithEarthquake(opts ...func(*EarthquakeQuery)) *EventTypeQuery {
	query := (&EarthquakeClient{config: etq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	etq.withEarthquake = query
	return etq
}
...
//		GroupBy(eventtype.FieldEarthquakeID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.EventType.Query().
//		Select(eventtype.FieldEarthquakeID).
//		Scan(ctx, &v)
...
func (etq *EventTypeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*EventType, error) {
	...
	if query := etq.withEarthquake; query != nil {
		if err := etq.loadEarthquake(ctx, query, nodes, nil,
			func(n *EventType, e *Earthquake) { n.Edges.Earthquake = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (etq *EventTypeQuery) loadEarthquake(ctx context.Context, query *EarthquakeQuery, nodes []*EventType, init func(*EventType), assign func(*EventType, *Earthquake)) error {
	ids := make([]int32, 0, len(nodes))
	nodeids := make(map[int32][]*EventType)
	for i := range nodes {
...
func (etq *EventTypeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(eventtype.Table, eventtype.Columns, sqlgraph.NewFieldSpec(eventtype.FieldID, field.TypeInt32))
	_spec.From = etq.sql
	...
	if fields := etq.ctx.Fields; len(fields) > 0 {
		...
		if etq.withEarthquake != nil {
			_spec.Node.AddColumnOnce(eventtype.FieldEarthquakeID)
		}
	}
