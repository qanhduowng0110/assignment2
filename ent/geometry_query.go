...
func (gq *GeometryQuery) QueryEarthquake() *EarthquakeQuery {
...
func (gq *GeometryQuery) FirstID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (gq *GeometryQuery) FirstIDX(ctx context.Context) int32 {
...
func (gq *GeometryQuery) OnlyID(ctx context.Context) (id int32, err error) {
	var ids []int32
...
func (gq *GeometryQuery) OnlyIDX(ctx context.Context) int32 {
...
func (gq *GeometryQuery) IDs(ctx context.Context) (ids []int32, err error) {
...
func (gq *GeometryQuery) IDsX(ctx context.Context) []int32 {
	ids, err := gq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (gq *GeometryQuery) Clone() *GeometryQuery {
	...
	return &GeometryQuery{
		...
		withEarthquake: gq.withEarthquake.Clone(),
...
func (gq *GeometryQuery) WithEarthquake(opts ...func(*EarthquakeQuery)) *GeometryQuery {
	query := (&EarthquakeClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withEarthquake = query
	return gq
}
...
//		GroupBy(geometry.FieldEarthquakeID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Geometry.Query().
//		Select(geometry.FieldEarthquakeID).
//		Scan(ctx, &v)
...
func (gq *GeometryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Geometry, error) {
	...
	if query := gq.withEarthquake; query != nil {
		if err := gq.loadEarthquake(ctx, query, nodes, nil,
			func(n *Geometry, e *Earthquake) { n.Edges.Earthquake = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GeometryQuery) loadEarthquake(ctx context.Context, query *EarthquakeQuery, nodes []*Geometry, init func(*Geometry), assign func(*Geometry, *Earthquake)) error {
	ids := make([]int32, 0, len(nodes))
	nodeids := make(map[int32][]*Geometry)
	for i := range nodes {
...
func (gq *GeometryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(geometry.Table, geometry.Columns, sqlgraph.NewFieldSpec(geometry.FieldID, field.TypeInt32))
	_spec.From = gq.sql
	...
	if fields := gq.ctx.Fields; len(fields) > 0 {
		...
		if gq.withEarthquake != nil {
			_spec.Node.AddColumnOnce(geometry.FieldEarthquakeID)
		}
	}
