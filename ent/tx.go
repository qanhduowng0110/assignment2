...
func (tx *Tx) init() {
	tx.APIRequestLog = NewAPIRequestLogClient(tx.config)
	tx.AssociatedEvent = NewAssociatedEventClient(tx.config)
	tx.Earthquake = NewEarthquakeClient(tx.config)
	tx.Earthquake_db = NewEarthquakeDbClient(tx.config)
	tx.EventType = NewEventTypeClient(tx.config)
	tx.FeatureType = NewFeatureTypeClient(tx.config)
	tx.FeltReport = NewFeltReportClient(tx.config)
...
// applies a query, for example: APIRequestLog.QueryXXX(), the query will be executed
